            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    ...
                    <form @submit.prevent="handleSubmit">
                        <div v-if="generalError" class="p-2 mb-6 border border-red-300 bg-red-50 text-red-700 rounded">
                            {{ generalError }}
                        </div>
                        <label for="email1" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Email</label>
                        <InputText id="email1" type="text" placeholder="Email address" class="w-full md:w-[30rem] mb-2" v-model="email" :class="{'p-invalid': !!errors.email}" />
                        <small v-if="errors.email" class="text-red-500 mb-6 inline-block">{{ errors.email }}</small>

                        <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                        <Password id="password1" v-model="password" placeholder="Password" :toggleMask="true" class="mb-2" fluid :feedback="false" :class="{'p-invalid': !!errors.password}"></Password>
                        <small v-if="errors.password" class="text-red-500 mb-6 inline-block">{{ errors.password }}</small>
